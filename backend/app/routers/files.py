...
import uuid
import magic
from fastapi import APIRouter, UploadFile, File, HTTPException, Depends, Path, Request  # ✅ Add Request
  # ✅ Add this import
...
async def upload_file(
    ...
    try:
        item = dynamo.create_file(
            user_id=user_id, file_name=safe_filename,
            s3_key=s3_key, file_type=file_type, file_size=len(file_bytes),
        )
    except Exception:
...
# ── GET /files ─────────────────────────────────────────────
@router.get("", response_model=FileListResponse)          # ✅ response_model added
...
# ── GET /files/{file_id} ───────────────────────────────────
@router.get("/{file_id}", response_model=FileDetailModel)  # ✅ response_model added
def get_file(
